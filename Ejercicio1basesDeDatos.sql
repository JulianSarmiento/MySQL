...
SELECT nombre, format(sal_emp,0) 
FROM empleados;
    
-- 5. Listar todas las comisiones.
SELECT format(comision_emp,0) as Comisiones 
FROM empleados;
...
SELECT nombre, cargo_emp, format(sal_emp, 0) 
FROM empleados 
...
SELECT nombre, format(sal_emp, 0) AS Salario, format(comision_emp, 0) AS Comisiones 
FROM empleados 
WHERE id_depto LIKE 2000 ORDER BY id_depto ASC;
...
SELECT nombre AS Nombre, format((sal_emp + comision_emp + 500),0) AS Total_a_pagar 
FROM empleados 
WHERE id_depto LIKE 3000 ORDER BY nombre ASC;

-- 13. Muestra los empleados cuyo nombre empiece con la letra J.
SELECT nombre AS Nombre 
FROM empleados 
WHERE nombre LIKE 'J%' ORDER BY nombre ASC;
...
SELECT nombre AS Nombre, format(sal_emp,0) AS Salario, format(comision_emp,0) AS Comisión, format((sal_emp + comision_emp),0) AS Salario_total 
FROM empleados 
...
SELECT nombre AS Nombre, format(sal_emp,0) AS Salario, format(comision_emp,0) AS Comisión
FROM empleados
...
SELECT format(max(sal_emp),0) AS Salario
FROM empleados;
...
SELECT format(max(sal_emp),0) AS Salario_máximo, format(min(sal_emp),0) AS Salario_mínimo, 
format(max(sal_emp) - min(sal_emp),0)  AS Diferencia_salarios
FROM empleados;

-- 24. Hallar el salario promedio por departamento.
SELECT format(AVG(sal_emp),0) AS Salario_promedio , id_depto
FROM empleados
GROUP BY id_depto;

										-- Consultas con Having
-- 25. Hallar los departamentos que tienen más de tres empleados. Mostrar el número de empleados de esos departamentos.
SELECT id_depto, COUNT(id_emp) AS Empleados 
FROM empleados
...
SELECT nombre AS Nombre_de_empleado, nombre_depto AS Departamento, nombre_jefe_depto AS Jefe 
FROM empleados
...
SELECT nombre AS Empleado, sal_emp AS Salario 
FROM empleados
